            ...
            try:
                ...
                if not recent_data.empty:
                    ...
                    if len(recent_data) >= 7:
                        # 過去7日間の平均変化を計算
                        all_data_for_week = db.get_measurements()
                        if not all_data_for_week.empty:
                            week_data = filter_data_by_period(all_data_for_week, 7)
                            if len(week_data) >= 2:
        ...
        try:
            ...
            if stats:
                ...
                with col1:
                    st.metric(
                        "平均体重",
                        f"{stats['weight_avg']:.1f} kg",
                        f"{stats['weight_change']:+.1f} kg"
                    )
                    st.metric("最小体重", f"{stats['weight_min']:.1f} kg")
                    st.metric("記録数", f"{stats['count']} 回")
                
                with col2:
                    st.metric("最大体重", f"{stats['weight_max']:.1f} kg")
                    st.metric("最新体重", f"{stats['weight_latest']:.1f} kg")
                    
                    # トレンド表示
                    trend_emoji = "📈" if stats['trend'] == 'up' else "📉" if stats['trend'] == 'down' else "➡️"
                    trend_text = "上昇" if stats['trend'] == 'up' else "下降" if stats['trend'] == 'down' else "横ばい"
                    st.metric("トレンド", f"{trend_emoji} {trend_text}")
                
                # 体脂肪率の統計（データがある場合のみ）
                if 'body_fat_avg' in stats:
                    st.markdown("#### 🔥 体脂肪率統計")
                    bf_col1, bf_col2 = st.columns(2)
                    
                    with bf_col1:
                        st.metric("平均体脂肪率", f"{stats['body_fat_avg']:.1f} %")
                        st.metric("最小体脂肪率", f"{stats['body_fat_min']:.1f} %")
                    
                    with bf_col2:
                        st.metric("最大体脂肪率", f"{stats['body_fat_max']:.1f} %")
                        if stats['body_fat_latest']:
                            st.metric("最新体脂肪率", f"{stats['body_fat_latest']:.1f} %")
                ...
                if stats['count'] > 1:
                    ...
                    # 平均日変化量
                    daily_change = stats['weight_change'] / stats_period
                    
                    change_col1, change_col2 = st.columns(2)
                    
                    with change_col1:
                        st.metric("総変化量", f"{stats['weight_change']:+.1f} kg")
                        st.metric("平均日変化", f"{daily_change:+.2f} kg/日")
                    
                    with change_col2:
                        # 変化率
                        change_rate = (stats['weight_change'] / stats['weight_oldest']) * 100
                        st.metric("変化率", f"{change_rate:+.1f} %")
                        
                        # 週間予測
                        weekly_prediction = daily_change * 7
                        st.metric("週間予測", f"{weekly_prediction:+.1f} kg/週")
                
            else:
                st.info(f"📝 直近{stats_period}日間のデータがありません")
        except Exception as e:
                ...
                if missing_columns:
                    ...
                    if not valid_rows:
                        ...
                        if duplicate_dates:
                            ...
                            st.write("重複する日付:", unique_duplicates[:10])  # 最初の10件のみ表示
                            ...
                            duplicate_action = "スキップ"  # 重複がない場合はスキップ
                                ...
                                for i, row_data in enumerate(valid_rows):
                                    try:
                                        ...
                                        if db.add_measurement(date_str, weight, body_fat):
                                            success_count += 1
                                            existing_dates.add(date_str)  # 重複チェック用に追加
                                        else:
                                            error_count += 1
    ...
    with col2:
        ...
        try:
            ...
            if not df.empty:
                ...
                # 選択期間の統計情報
                if not df.empty:
                    st.subheader("📊 選択期間の統計情報")
                    
                    col_stat1, col_stat2, col_stat3, col_stat4 = st.columns(4)
                    
                    with col_stat1:
                        st.metric("データ数", f"{len(df)}件")
                    ...
                    with col_stat3:
                        if len(df) > 1:
                            ...
                            st.metric("期間内変化", f"{change:+.1f}kg")
                        else:
                            st.metric("期間内変化", "N/A")
                    
                    with col_stat4:
                        weight_range = df['weight'].max() - df['weight'].min()
                        st.metric("変動幅", f"{weight_range:.1f}kg")
        ...
        try:
            df_recent = db.get_measurements(20)  # 最新20件を取得
            ...
            if not df_recent.empty:
                ...
                # 編集可能なデータエディタ
                st.info("💡 データをダブルクリックで編集できます。行を削除するには、行の左端にあるゴミ箱アイコンをクリックしてください。")
                ...
                if st.session_state.get('show_delete_all_confirm', False):
                    ...
                    # 削除に関する注意事項
                    st.info("⚠️ 個別削除: 行の左端にあるゴミ箱アイコンをクリックしてから「変更を保存」ボタンを押してください。")
                    
                    if st.button("🗑️ 全データを削除", key="show_delete_all"):
                        st.session_state['show_delete_all_confirm'] = True
                        st.rerun()
                
            else:
                st.info("📝 まだデータがありません。左側のフォームから記録を追加してください。")
